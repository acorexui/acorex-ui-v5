...
export class AXSchedulerComponent {
  ...
  ngAfterViewInit(): void {
    setTimeout(_ => {
      ...
      //
      this.dataSource.onDataReceived.subscribe((e:any) => {
        this.events = e;
      ...
      //
      if (this.toolbarView) {
        this.toolbarView.onViewChanged.subscribe((c: any) => {
          this.currentView = c;
        });
      ...
      //
      if (this.toolbarNavigator) {
        this.toolbarNavigator.onNavigate.subscribe((c: any) => {
          if (c === 'next') {
            this.view.next();
          }
          else if (c === 'prev') {
      ...
      if (this.searchInput) {
        this.searchInput.onValueChanged.subscribe((c: any) => {
          this.searchText = c;
          if (c) {
